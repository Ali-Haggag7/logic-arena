...
export const ProtocolForm = ({ newScriptTitle, setNewScriptTitle, isLoading, onSubmit, isGuest }: ProtocolFormProps) => {
    ...
    return (
        <div className="sticky top-4 md:top-8 z-30">
            {/* Mobile Layout */}
            <div className={`md:hidden bg-card border border-accent/50 rounded-2xl overflow-hidden transition-all duration-300 ease-in-out shadow-lg ${isExpanded ? "max-h-[300px]" : "max-h-[60px]"}`}>
                {!isExpanded ? (
                    <button
                        type="button"
            ...
            {/* Desktop Layout */}
            <div className="hidden md:block bg-card/60 backdrop-blur-xl border border-accent/10 rounded-xl p-6 shadow-[var(--card-shadow)] relative">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-accent to-transparent opacity-50"></div>
                <h3 className="text-accent uppercase tracking-widest text-xs font-bold mb-6 flex items-center gap-2">
                    [+] CREATE NEW SCRIPT
                </h3>
                <form onSubmit={onSubmit} className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-[10px] text-text-secondary uppercase tracking-[0.2em] font-bold ml-1">
                            SCRIPT NAME
                        </label>
                        <input
                            type="text"
                            placeholder="Enter script name..."
                            className="w-full bg-bg-secondary/50 border border-accent/10 rounded-lg p-3 text-accent outline-none focus:border-accent focus:bg-accent/5 transition-all shadow-inner text-xs placeholder-text-secondary/40"
                            value={newScriptTitle}
                            onChange={(e) => setNewScriptTitle(e.target.value)}
                            required

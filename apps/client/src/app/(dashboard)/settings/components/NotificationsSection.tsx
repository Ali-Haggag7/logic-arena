...
export function NotificationsSection({ isGuest = false }: { isGuest?: boolean }) {
  ...
  const ITEMS = [
    { id: "challenges", label: "Challenge Requests", sub: "Incoming combat challenges from other operators", val: challengeReqs, set: (v: boolean) => { if (!isGuest) { setChallengeReqs(v); save("notif_challenges", v); } } },

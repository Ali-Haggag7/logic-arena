    ...
    <div className="fixed inset-0 z-[120] grid place-items-end bg-bg-primary/85 px-0 backdrop-blur-sm md:place-items-center md:px-4">
      ...
      >
        <div className="flex items-start justify-between gap-4">
          ...
          <button type="button" aria-label="Close feature request details" title="Close" onClick={onClose} className="cursor-pointer grid min-h-11 min-w-11 place-items-center rounded-lg border border-accent/20 text-text-secondary transition-colors hover:border-accent hover:text-accent">
            <X className="h-5 w-5" />
          </button>
        </div>
      ...
      render: (value, row) => {
        ...
        return (
          <button
            ...
            className="cursor-pointer min-h-11 w-full min-w-56 rounded-lg border border-accent/20 bg-bg-primary px-3 text-left text-sm font-black text-text-primary transition-colors hover:border-accent hover:text-accent"
          >
          ...
          <div className="flex min-w-56 flex-wrap items-center gap-2">
            ...
            <div className="w-48">
              <CyberSelect
                ...
                onChange={(val) => void handleStatusChange(request, val as FeatureRequestStatus)}
                options={STATUSES.map((option) => ({ value: option, label: formatLabel(option) }))}
              />
            </div>
            <button type="button" aria-label={`Delete ${request.title}`} title="Delete" disabled={deleteState.isLoading} onClick={() => void handleDelete(request)} className="cursor-pointer grid min-h-11 min-w-11 place-items-center rounded-lg border border-[var(--sem-danger)] bg-[rgba(var(--sem-danger-rgb),0.1)] text-[var(--sem-danger)] transition-colors disabled:opacity-50">

...
export function ThemeSwitcher() {
  ...
  return (
    <div className="px-[10px] pb-3 relative z-10">
      <div className="text-[7px] tracking-[0.22em] text-accent/25 font-bold px-1 pb-1.5 uppercase">
        theme
      </div>
        ...
        {THEMES.map(({ id, label, icon }) => {
          ...
          return (
            <button
              ...
              onClick={() => setTheme(id)}
              title={label}
              className={`flex-1 flex flex-col items-center gap-0.5 py-1.5 rounded-md text-[10px] font-mono font-bold tracking-[0.05em] border transition-all duration-200 cursor-pointer ${
                isActive

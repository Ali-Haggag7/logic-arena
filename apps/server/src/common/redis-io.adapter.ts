...
function redisTlsOptions(): { tls?: Record<string, never> } {
    const enabled = ['1', 'true', 'yes'].includes((process.env.REDIS_TLS ?? '').toLowerCase());
    return enabled ? { tls: {} } : {};
}
...
export class RedisIoAdapter extends IoAdapter {
    ...
    async connectToRedis(): Promise<void> {
        ...
        try {
            ...
            this.logger.error(`Socket.io Redis adapter unavailable; using local adapter fallback: ${message}`);
